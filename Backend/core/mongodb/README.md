...
#### **5. Search với Filters**atabase_name
     ```
   - Contact project lead để lấy credentials nếu chưa có
...
### **10 Tháng 12, 2025: Module Refactoring & Simplification**

**Vấn đề:** 
1. MongoDB initialization quá phức tạp với Config class
2. Connection manager nằm riêng folder `core.database`
3. Có unused code (`restaurant.py`)

**Cách làm:**

1. **Di chuyển `connection.py` vào module:**
   ```
   OLD: Backend/core/database/mongodb.py
   NEW: Backend/core/mongodb/connection.py
   ```
   - Tất cả MongoDB logic trong 1 folder
   - Import đơn giản: `from core.mongodb import MongoDB`

2. **Xóa unused files:**
   - ❌ `Backend/core/models/restaurant.py` - Không được dùng
   - Handler đã có schemas riêng (`MongoDBRestaurantResponse`)

3. **Đơn giản hóa initialization:**
   ```python
   # Before (20 dòng)
   ...
   await MongoDB.initialize(mongo_config)
   
   # After (3 dòng)
   from core.mongodb import MongoDB
   await MongoDB.initialize()  # Auto-reads MONGODB_CONNECTION_STRING from .env
   ```

**Lợi ích:**
- ✅ Module cohesion: Connection + Handlers + Docs trong 1 folder
- ✅ Clean imports: `from core.mongodb import MongoDB, MongoDBHandlers`
- ✅ Giảm 85% initialization code (từ 20 → 3 dòng)
- ✅ Xóa dead code và dependencies không cần thiết
- ✅ Tuân thủ 12-factor app (environment variables)

**Files changed:**
- `Backend/core/mongodb/connection.py` - Moved from `database/mongodb.py`
- `Backend/core/mongodb/__init__.py` - Export MongoDB, MongoConfig
- `Backend/app.py` - Simplified imports
- ❌ Deleted `core/models/restaurant.py` and `core/database/mongodb.py`
...
```
Backend/core/mongodb/
├── __init__.py           # Export: MongoDB, MongoConfig, Handlers, Schemas
├── connection.py         # MongoDB connection manager (Singleton)
├── handlers.py           # Restaurant search handlers (Object pattern)
...
- Auto-initialize từ `MONGODB_CONNECTION_STRING` env variable
- Provide database instance: `MongoDB.get_database()`
- Manage indexes creation
- Handle async (runtime) và sync (import) clients

**Key Classes:**
- `MongoConfig` - Configuration model (optional)
...
await MongoDB.initialize()        # Auto-reads from .env
db = MongoDB.get_database()       # Get database instance
await MongoDB.create_indexes()    # Create performance indexes
await MongoDB.close()             # Cleanup on shutdown
```

#### **`handlers.py` - Search Operations** 🔍
- **Object pattern** - Create new instance per request
- 4 public methods: Search, SearchNearby, SearchByText, GetTopRated
- Dual strategy: Text search + Geospatial search
- Haversine formula for distance calculation

**Key Classes:**
- `MongoDBSearchInputSchema` - Input validation
- `MongoDBRestaurantResponse` - Single restaurant response
- `MongoDBSearchResponse` - Search result wrapper
- `MongoDBHandlers` - Core handler class
...
from core.mongodb import (
    ...
    MongoConfig,                  # Config (optional)
    MongoDBHandlers,              # Search handlers
    MongoDBSearchInputSchema,     # Input schema
    MongoDBRestaurantResponse,    # Single result
    MongoDBSearchResponse         # Response wrapper
)
```
...
2. **Tạo import script đơn giản:** `Data/scripts/simple_import.py`
   - Đọc CSV: `VietnamRestaurantsWithTags.csv` (48,757 records)
...
#### **1. Setup & Initialize (trong app.py)**
...
# Initialize MongoDB connection (tự động đọc MONGODB_CONNECTION_STRING từ .env)
...
**✅ Phần này đã được setup sẵn trong `app.py`!** Bạn không cần làm gì thêm.

---

#### **2. Sử dụng Handler trong Router/Service**
...
# Create handler (Object pattern - mỗi request tạo mới)
handler = MongoDBHandlers(db)

# Sử dụng handler để search
result = await handler.Search(inputs)
```

**Pattern:**
- ✅ **Object Pattern**: Mỗi request tạo instance mới `MongoDBHandlers(db)`
- ✅ Database connection được cache (singleton) qua `MongoDB.get_database()`
- ✅ Handler instance lightweight - không tốn nhiều resource

---

#### **3. Search với Text (Tìm món ăn)**
...
**📖 Chi tiết:** Xem examples trong file này
...
### 1. Setup (Đã có sẵn trong app.py)

```python
# Backend/app.py - onInitialize()
from core.mongodb import MongoDB

# Initialize MongoDB (auto-reads from .env)
await MongoDB.initialize()
await MongoDB.create_indexes()
```

**✅ Bạn không cần làm bước này - đã được setup sẵn!**

### 2. Sử dụng Handler trong code
...
1. **Handler Pattern**: Object pattern - Mỗi request tạo instance mới `MongoDBHandlers(db)`
2. **Database Connection**: Singleton - `MongoDB.get_database()` return cached connection
3. **MinRating = None**: Nếu không truyền `MinRating`, sẽ lấy tất cả ratings
4. **Radius**: Đơn vị là **meters** (5000 = 5km)
5. **Text Search**: Tự động search trong `name`, `category`, `address`, `tags`
6. **Distance Calculation**: Dùng Haversine formula (chính xác cho Earth sphere)
7. **Sorting**: 
   - Có text: Sort by **text score** → distance
   - Không có text: Sort by **distance** → rating
8. **Initialization**: `MongoDB.initialize()` tự động đọc `MONGODB_CONNECTION_STRING` từ `.env`
